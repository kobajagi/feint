...
Version **0.3.1**, named rather than fetched as `latest`: a mutable reference
...
```bash
base=https://github.com/stephrobert/feint/releases/download/v0.3.1
...
```bash
gh release download v0.3.1 --repo stephrobert/feint --pattern 'feint-linux-amd64'
...
91 routes are mounted across the three packs. The tables count *upstream operations*
...
20 routes mounted, 7% of the measured upstream surface served.
...
14 routes mounted, 3% of the measured upstream surface served.
